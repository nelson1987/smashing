...
namespace Smashing.Tests.Integrations
{
    public class MovementControllerIntegrationTests
    {
        ...
        {
            // Act
            var content = new StringContent(System.Text.Json.JsonSerializer.Serialize(_command),
        ...
        {
            var content = new StringContent(System.Text.Json.JsonSerializer.Serialize(_command with { Valor = 0 }),
                Encoding.UTF8, "application/json");
            var result = await Client.PostAsync($"api/Movement", content);
